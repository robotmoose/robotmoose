...
if __name__=="__main__":
	try:
		...
		parser=argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
			...
			Makes robots from a list of robots on superstar.
		...
		def do_make(data):
			...
			ss.set("/robots/"+robot_name,robot,auth,onsuccess,onerror)
			if len(robot_auth)>1:
		...
		for robot in robots:
			next_robot=robot
			ss.get("/robots/"+robot[0],do_make,onerror)
			ss.flush()
