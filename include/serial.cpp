/*  SerialPort port object for use with wxWidgets
 *  Copyright 2010, Paul Stoffregen (paul@pjrc.com)
 *  Hacked to remove Wx by Orion Lawlor (lawlor@alaska.edu) 2012-02-14
 *
...
#ifndef _ /* used to internationalize string constants */
#  define _(str) str
#endif
...
int SerialPort::Open(const std::string& name)
{
	...
	if (port_fd < 0) {
		if (errno == EACCES) {
			...
			error_msg = _("Unable to open ") + name +
				_(", SerialPort port hardware not installed");
		} else if (errno == ENOENT) {
			error_msg = _("Unable to open ") + name +
				_(", Device name does not exist");
		} else {
			error_msg = _("Unable to open ") + name +
				_(", ") + std::string(strerror(errno));
		}
		return -1;
	}
...
std::string SerialPort::get_name(void)
{
...
std::string SerialPort::error_message(void)
{
	return error_msg;
}

int SerialPort::Is_open(void)
{
	return port_is_open;
}
...
// set the baud rate
int SerialPort::Set_baud(int baud)
{
	if (baud <= 0) return -1;
	//printf("set_baud: %d\n", baud);
	baud_rate = baud;
...
int SerialPort::Read(void *ptr, int count)
{
	if (!port_is_open) return -1;
	if (count <= 0) return 0;
#if defined(LINUX)
...
int SerialPort::Write(const void *ptr, int len)
{
	//printf("Write %d\n", len);
	if (!port_is_open) return -1;
#if defined(LINUX) || defined(MACOSX)
...
int SerialPort::Input_wait(int msec)
{
	if (!port_is_open) return -1;
#if defined(LINUX) || defined(MACOSX)
	fd_set rfds;
	...
	FD_ZERO(&rfds);
	FD_SET(port_fd, &rfds);
	int err=select(port_fd+1, &rfds, NULL, NULL, &tv);
	return err;
#elif defined(WINDOWS)
...
void SerialPort::Input_discard(void)
{
	if (!port_is_open) return;
#if defined(LINUX) || defined(MACOSX)
...
void SerialPort::Output_flush(void)
{
	if (!port_is_open) return;
#if defined(LINUX) || defined(MACOSX)
	tcdrain(port_fd);
...
int SerialPort::Set_control(int dtr, int rts)
{
	if (!port_is_open) return -1;
#if defined(LINUX) || defined(MACOSX)
...
std::vector<std::string> SerialPort::port_list()
{
	...
	while ((f = readdir(dir)) != NULL) {
		...
		if (fd < 0) {
			...
			// problem)
			if (errno == EACCES) list.push_back(std::string(s));
	...
	// http://developer.apple.com/documentation/DeviceDrivers/Conceptual/WorkingWSerialPort/WWSerialPort_SerialPortDevs/chapter_2_section_6.html
	...
	if (ret) {
		...
		printf("Detect SerialPort using brute force GetDefaultCommConfig probing: ");
		for (int i=1; i<=32; i++) {
			printf("try  %s", buf);
			COMMCONFIG cfg;
			DWORD len;
