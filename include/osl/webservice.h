...
*/
OSL_DLL std::string download_url(std::string URL,network_progress *p=0);
inline OSL_DLL std::string download_url(std::string URL,network_progress &p)
	{ return download_url(URL,&p); }
...
public:
	http_connection(std::string host,network_progress *p=0,int port=80,int timeoutSeconds=10);
	http_connection(std::string host,network_progress &p,int port=80,int timeoutSeconds=10);
