...
/** Send an STL string on this socket. */
inline void skt_send(SOCKET skt,const std::string &s) 
{
	skt_sendN(skt,&s[0],s.size());
}
...
inline std::ostream &operator<<(std::ostream &o,const skt_ip_t &ip) {
	o<<skt_print_ip(ip);
	return o;
}
