...
osl::network_progress::~network_progress() {}
...
// Make HTTP connection to this TCP port
void osl::http_connection::connect(int port,int timeout)
{
	if (p) p->status(1,"Looking up IP address for "+host);
	skt_ip_t hostIP=skt_lookup_ip(host.c_str());
	if (p) p->status(1,"Connecting to "+host);
...
{
	if (p) p->status(1,"Sending HTTP request to "+host);
	if (p) p->status(3,"HTTP request data: "+data);
	skt_sendN(s,&data[0],data.size());
	
	if (p) p->status(1,"Waiting for HTTP response from "+host);
	std::string status=skt_recv_line(s);
	if (p) p->status(2,"HTTP response status: "+status);
	...
	{   /* ^ a zero-length line indicates the end of the HTTP headers */
		if (p) p->status(3,"HTTP response header line: "+l);
...
std::string osl::http_connection::receive(void) 
{
	...
	if (p) p->status(1,"Retrieving "+int2str(length/1024)+" KiB of HTTP data from "+host);
	...
	for (int start=0;start<length;start+=chunkSize) {
		...
		if (p) p->status(2,"Retrieved "+int2str((start+amt)/1024)+" KiB so far from "+host);
	}
	if (length<1024)
		if (p) p->status(3,"Incoming data: "+data);
	return data;
}
...
std::string osl::http_connection::receive_header(const std::string &keyword)
{
	...
	if (p) p->status(2,"HTTP header keyword "+keyword+" has value "+value);
	return value;
}
