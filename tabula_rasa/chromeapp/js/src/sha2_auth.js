...
the password never crosses the network, only the hash.

We use the SHA-256 hash implementations from CryptoJS:
	https://crypto-js.googlecode.com/files/CryptoJS%20v3.1.2.zip

Assembled by Dr. Orion Lawlor, lawlor@alaska.edu, 2014-11-17 (Public Domain)
*/

/* CryptoJS v3.1.2 core.js
...
Finally, this is the real authentication generation code.
	...
	Message is the new value to authenticate
	seq is a sequence number (base-10 integer) used to prevent replay attacks.
The return value is an alphanumeric version of the hash.
*/
function getAuthCode(password,path,message,seq) {
	...
	var auth="";
	var debug="";
	for (var i=0;i+4<=hash.length;i+=4) {
		var n=parseInt(hash.slice(i,i+4),16); // eat 4 hex digit chunk
		if (n<lastcopy) auth += table.charAt(n%table.length);
		// else skip this part of the key (avoids bias toward low letters)
		debug+="<br> "+n+" "+auth;
	}
	// return hash + debug + " -> " + auth;
	return auth;
}
