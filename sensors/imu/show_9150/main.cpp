...
public:
	...
	// Update 3D acceleration
		accel=world_from_local((accel_raw)*(9.8/16000.0)); // units: meter/sec^2
	...
	// Update position (lurchy and drunken!)
		...
		origin.x*=0.9; // scale back to reduce drifting
		origin.y=(origin.y+1.0)*0.99-1.0;
	...
	// Update serial data
		while (Serial.available()) {
			...
			if (c=='\n') { // end of line
				...
				serial_line="";
			}else if (c==-1 || c=='\r') { // silly DOS newline
			}
			else serial_line+=c;
		}
	}
	...
	void process_serial(const std::string &line) {
		...
		if (format!="A") std::cerr<<"Unrecognized line format '"<<line<<"' -> '"<<format<<"'\n";
		else {
			accel_raw=read_xyz(ss);
			accel_raw=-vec3(accel_raw.x,accel_raw.y,accel_raw.z);
