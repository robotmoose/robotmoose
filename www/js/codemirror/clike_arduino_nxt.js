...
(function() {
  ...
  mimes(["text/x-arduino"], {
    name: "clike",
    keywords: words(cKeywords + " random randomSeed min max abs constrain map pow sqrt sin cos tan " +
                    ...
                    "wchar_t"),
    blockKeywords: words("catch class do else finally for if struct switch try while"),
    atoms: words("true false null HIGH LOW OUTPUT INPUT A0 A1 A2 A3 A4 A5"),
    hooks: {"#": cppHook}
  });
    mimes(["text/x-robotc"], {
    name: "clike",
    keywords: words(cKeywords + " task main wait1Msec motor SensorValue asm dynamic_cast namespace reinterpret_cast try bool explicit new " +
                    "static_cast typeid catch operator template typename class friend private " +
                    "this using const_cast inline public throw virtual delete mutable protected " +
                    "wchar_t"),
    blockKeywords: words("catch class do else finally for if struct switch try while"),
